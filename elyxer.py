...
class GeneralConfig(object):
  ...
  version = {
      u'date':u'2013-03-10', u'lyxformat':u'413', u'number':u'1.2.5', 
      }
...
class StyleConfig(object):
  ...
  quotes = {
      ...
      u'prs':u'’', u'sld':u'”', u'srd':u'”', u'qrd':u'"', 
      }
...
# let \eqref latex commands pass through without modification
      oldway = True
      if Options.mathjax and element.__class__.__name__ == 'Reference':
        key = element.getparameter('reference')
        formatkey = element.getparameter('LatexCommand')
        if formatkey == 'eqref':
          myhtml = '\\eqref{' + key + '}'
          html += myhtml
          oldway = False
...
# gerard additions start
# add equation numbering with AMS style
        html.append(u'<script type="text/x-mathjax-config">\n')
        html.append(u'MathJax.Hub.Config({\n')
        html.append(u'  TeX: { equationNumbers: { autoNumber: "AMS" } }\n')
        html.append(u'});\n')
...
# gerard additions end
        ...
        html.append(u'</script>\n')
# gerard additions start
# override lyx.css style sheet to show MathJax processed latex equations in black
        html.append(u'<style>\n')
        html.append(u'span.MathJax_Preview {\n')
        html.append(u'  color: black;\n')
        html.append(u'}\n')
        html.append(u'</style>\n')
# gerard additions end
      else:
...
# gerard additions start
# allow output of macros when using MathJax
#    self.output = EmptyOutput()
    if Options.mathjax:
      self.output = TaggedOutput().settag('span class="formula"')
    else:
